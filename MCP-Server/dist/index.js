...
// ---------- TOOLS ----------
// 1) /posts/ideas
server.registerTool("posts_ideas", {
    title: "Generate LinkedIn post ideas",
    description: "Calls the /posts/ideas endpoint on the LinkedIn post agent. " +
        "Pass the exact JSON body that the backend expects in `body`.",
    // Let the agent pass any JSON – your backend will validate
    inputSchema: {
        body: z.any().describe("JSON body for /posts/ideas. Use the same fields as IdeasInput in the Java service.")
    }
}, async ({ body }) => {
...
// 2) /posts/outline
server.registerTool("posts_outline", {
    title: "Generate outline for a LinkedIn post",
    description: "Calls the /posts/outline endpoint. Provide OutlineInput JSON in `body`.",
    inputSchema: {
        body: z.any().describe("JSON body for /posts/outline (OutlineInput).")
    }
}, async ({ body }) => {
...
// 3) /posts/draft
server.registerTool("posts_draft", {
    title: "Generate full LinkedIn post draft",
    description: "Calls the /posts/draft endpoint. Provide DraftInput JSON in `body`.",
    inputSchema: {
        body: z.any().describe("JSON body for /posts/draft (DraftInput).")
    }
}, async ({ body }) => {
...
// 4) /posts/polish
server.registerTool("posts_polish", {
    title: "Polish an existing LinkedIn post",
    description: "Calls the /posts/polish endpoint. Provide PolishInput JSON in `body`.",
    inputSchema: {
        body: z.any().describe("JSON body for /posts/polish (PolishInput).")
    }
}, async ({ body }) => {
...
// 5) /posts/hashtagize
server.registerTool("posts_hashtagize", {
    title: "Generate hashtags for a post",
    description: "Calls the /posts/hashtagize endpoint. Provide HashtagizeInput JSON in `body`.",
    inputSchema: {
        body: z.any().describe("JSON body for /posts/hashtagize (HashtagizeInput).")
    }
}, async ({ body }) => {
...
server.registerTool("posts_image_prompts", {
    title: "Generate image prompts for the post",
    description: "Calls the /posts/image-prompts endpoint. Provide ImagePromptsInput JSON in `body`.",
    inputSchema: {
        body: z.any().describe("JSON body for /posts/image-prompts (ImagePromptsInput).")
    }
}, async ({ body }) => {
...
// 7) /posts/package
server.registerTool("posts_package", {
    title: "Package text + hashtags + image prompt",
    description: "Calls the /posts/package endpoint. Provide PackageInput JSON in `body`.",
    inputSchema: {
        body: z.object({
...
server.registerTool("posts_full", {
    ...
    description: "High-level tool that orchestrates multiple steps in the Java backend. " +
